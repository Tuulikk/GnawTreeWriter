...
fn extract_calls(tree: &TreeNode, file_path: &str, lines: &[&str], relations: &mut Vec<Relation>) {
    // First: collect all defined function names in this file
    ...
    // Then: find calls to those functions (and external calls via ::)
...
fn find_calls_in_scope(tree: &TreeNode, defined: &std::collections::HashSet<String>, file_path: &str, lines: &[&str], relations: &mut Vec<Relation>) {
    if tree.node_type == "call_expression" || tree.node_type == "call"
        ...
        for child in &tree.children {
            if child.node_type == "identifier" {
                ...
                // Track calls to locally-defined functions OR module::function patterns
                if defined.contains(&callee) || callee.contains("::") {
                    relations.push(Relation {
                        from: String::new(), // Will be set by caller context
                        to: callee,
    ...
    for child in &tree.children {
        find_calls_in_scope(child, defined, file_path, lines, relations);
    }
}
