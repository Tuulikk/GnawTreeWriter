...
pub fn index_relations(file_path: &str) -> Result<RelationIndex> {
    ...
    eprintln!("[DEBUG] file={} total={} imports={} calls={} uses={} impls={}",
        file_path, relations.len(),
        summary.get("imports").unwrap_or(&0),
        summary.get("calls").unwrap_or(&0),
        summary.get("uses").unwrap_or(&0),
        summary.get("implements").unwrap_or(&0),
    );
...
fn extract_import_target(import_line: &str) -> Option<String> {
    ...
    fn extract_calls(tree: &TreeNode, file_path: &str, lines: &[&str], relations: &mut Vec<Relation>) {
        ...
        eprintln!("[DEBUG] defined functions ({}): {:?}", defined_funcs.len(), defined_funcs.iter().take(10).collect::<Vec<_>>());
        find_calls_in_scope(tree, &defined_funcs, file_path, lines, relations);
        eprintln!("[DEBUG] calls found: {}", relations.iter().filter(|r| r.relation_type == "calls").count());
    }
...
"#;
    ...
    fn test_debug_mcp_relations() {
        ...
        eprintln!("[DEBUG-MCP] total={} calls={} uses={} imports={}",
            result.relations.len(),
            result.summary.get("calls").unwrap_or(&0),
            result.summary.get("uses").unwrap_or(&0),
            result.summary.get("imports").unwrap_or(&0),
        );
        for r in result.relations.iter().filter(|r| r.relation_type == "calls").take(5) {
            eprintln!("  call: {} -> {}", r.from, r.to);
        }
        assert!(result.relations.len() < 100, "Too many relations: {}", result.relations.len());
    }
}
