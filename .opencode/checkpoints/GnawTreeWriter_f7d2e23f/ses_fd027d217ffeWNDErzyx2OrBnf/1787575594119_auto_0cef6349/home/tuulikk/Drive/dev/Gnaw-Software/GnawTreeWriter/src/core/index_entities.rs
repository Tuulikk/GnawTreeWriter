...
fn detect_visibility(node: &TreeNode) -> String {
    for child in &node.children {
        if child.node_type == "visibility_modifier" || child.node_type == "visibility" {
            // Check the modifier's own content first
            let content = child.content.trim();
            if !content.is_empty() && content != "unnamed" {
                return content.to_string();
            }
            // Check children of the visibility_modifier (e.g., "pub" child node)
    ...
    // Check if first token is "pub"
    let first = node.content.trim().chars().take(3).collect::<String>();
    if first == "pub" {
        let rest = node.content.trim().chars().skip(3).take(1).collect::<String>();
        if rest == "(" {
            let end = node.content.find(')').unwrap_or(4);
            return node.content.trim()[..=end].to_string();
        }
