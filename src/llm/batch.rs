...
#[derive(Debug, Clone)]
pub enum BatchEdit {
    Edit { node_path: String, content: String },
    Insert { parent_path: String, position: usize, content: String },
    Delete { node_path: String },
}

/// Apply multiple edits in a single operation
pub fn apply_batch(operation: BatchOperation) -> Result<BatchResult> {
    let writer = GnawTreeWriter::new(&operation.file_path)?;
    ...
    for edit in operation.operations {
        match edit {
            BatchEdit::Edit { node_path, content } => {
                match writer.edit(EditOperation::Edit { node_path: node_path.clone(), content: content.clone() }) {
                    Ok(_) => results.push(format!("Edited node: {}", node_path)),
                    Err(e) => failed.push((format!("Edit node: {}", node_path), e.to_string())),
                }
            }
            BatchEdit::Insert { parent_path, position, content } => {
                match writer.edit(EditOperation::Insert { parent_path: parent_path.clone(), position, content: content.clone() }) {
            ...
            BatchEdit::Delete { node_path } => {
                match writer.edit(EditOperation::Delete { node_path: node_path.clone() }) {
